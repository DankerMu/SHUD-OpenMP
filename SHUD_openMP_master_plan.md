...
<!-- IMG: fig01_three_phases_overview.png -->
<!-- 生图提示：三阶段流程图，从左到右。
  阶段一"预并行 S0–S6"（蓝色调）→ 产出 B1；
  阶段二"strict 并行 P1–P7"（绿色调）→ 产出 P-strict，标注"bitwise identical"；
  阶段三"production 并行 P8–P9"（橙色调）→ 产出 P-prod，标注"deterministic tolerance"。
  三个阶段之间用箭头 + 门控图标（🚦或闸门）连接，表示 go/no-go。
  风格：技术文档配图，干净简洁，白底，无 3D 效果。 -->
...
<!-- IMG: fig02_isolation_of_differences.png -->
<!-- 生图提示：2×2 矩阵或四象限图，标题"结果差异的四种来源"。
  四格分别是：
  ① 方程路径不一致（S0–S6 消除）
  ② 浮点加法顺序变化（P1–P7 控制）
  ③ 共享变量竞争/data race（P1–P7 消除）
  ④ 求解器行为变化（P8–P9 允许）
  下方一条时间轴，标注每个阶段隔离哪类差异。
  风格：技术文档配图，四格用不同色块区分，白底。 -->
...
<!-- IMG: fig07_baseline_relationships.png -->
<!-- 生图提示：四层基线关系图，纵向自上而下：
  B0（灰色，"原始快照"）
    ↓ S0–S6 重构（蓝色箭头，标注"bitwise identical 或 intentional diff"）
  B1（蓝色高亮，"parallel-ready 参考"）
    ↓ P1–P7（绿色箭头，标注"bitwise identical"）
  P-strict（绿色，"strict 并行结果"）
    ↓ P8–P9（橙色箭头，标注"deterministic tolerance"）
  P-prod（橙色，"production 并行结果"）
  右侧标注每层的精度等级：A0/A1 → A2/A3 → A4/A5。
  风格：层级关系图，白底，四色调对应四层。 -->
...
<!-- IMG: fig03_full_roadmap.png — 需替代下方 Mermaid -->
<!-- 生图提示：完整阶段路线图，纵向三组：
  上组"预并行"蓝色调：S0→S1→S2→S3→S4→S5→S6，每个节点标注简短名称；
  中组"strict 并行"绿色调：P1→P2→P3→P4→P5→P6→P7；
  下组"production 并行"橙色调：P8→P9；
  S6→P1 和 P7→P8 用加粗箭头 + 门控图标。
  S0 和 S6 节点高亮（B0/B1 里程碑），P7 高亮（strict 完成），P8/P9 高亮（production）。
  风格：技术路线图，白底，节点圆角矩形，三色分组。 -->
![完整路线图](figures/fig03_full_roadmap.png)

<details>
<summary>Mermaid 源码（待替代为生成图片）</summary>
...
<!-- IMG: fig04_simplified_roadmap.png — 需替代下方 Mermaid -->
<!-- 生图提示：极简横向流程图，五个节点从左到右：
  B0（灰）→ 预优化 S0-S6（蓝）→ B1（蓝高亮）→ Strict OpenMP P1-P7（绿）→ Production OpenMP P8-P9（橙）。
  B0→B1 之间标注"bitwise / intentional diff"，B1→P7 标注"bitwise identical"，P7→P9 标注"deterministic tolerance"。
  风格同上。 -->
...
<!-- IMG: fig05_rhs_dataflow.png -->
<!-- 生图提示：SHUD 单次 RHS 评估的数据流图。
  纵向自上而下六个阶段块：
  ① f_update（状态同步 + 清零）
  ② ET / snow / interception（element-local 垂向）
  ③ element lateral flux（surface/sub 横向）
  ④ segment-river flux（河段交换）+ river downflow
  ⑤ PassValue / gather（汇总到 river/element/lake）
  ⑥ f_applyDY（DY 组装）
  每个块内标注关键数组名（QeleSurf, QsegSurf, QrivDown, DY 等）。
  块之间用箭头表示数据依赖。
  用红色 ⚠ 标记当前 serial/OMP 有差异的块（①②④⑥），用红色闪电标记 shared write 位置（④⑤）。
  风格：技术架构图，白底，块状布局，色调与 fig03 一致。 -->
![RHS 数据流与问题标注](figures/fig05_rhs_dataflow.png)

<!-- IMG: fig06_serial_vs_omp_diff.png -->
<!-- 生图提示：左右对比表格或双列图，标题"serial 路径 vs OMP 路径差异一览"。
  左列"serial f_update/f_loop/f_applyDY"，右列"OMP f_update_omp/f_loop_omp/f_applyDY_omp"。
  每行一个差异点，用 ✓/✗ 或绿/红标记：
  - f_etFlux(): serial ✓, OMP ✗
  - lake vertical/horizontal: serial ✓, OMP ✗
  - lake evap clamp: serial ✓, OMP ✗
  - lake DY: serial ✓, OMP ✗
  - lake element DY=0: serial ✓, OMP ✗
  - river DY formula: serial=Length+clamp+dAtodY, OMP=u_TopArea
  - 负状态 clamp: serial 不 clamp, OMP clamp
  - Qe2r/QrivSurf 清零: serial 在 update, OMP 在 PassValue
  - area/isf/ius/igw: serial 局部变量, OMP shared(data race)
  风格：对比表格，红色高亮差异，白底。 -->
...
<!-- IMG: fig08_rhs_core_structure.png -->
<!-- 生图提示：RHS core 的模块化结构图。
  一个纵向流程，左侧标注 execution policy（Serial / StrictOMP / ProductionOMP）。
  七个模块块从上到下：
  ① rhs_reset_and_update_state
  ② rhs_element_vertical (ET, infiltration, recharge)
  ③ rhs_element_horizontal (surface/sub lateral)
  ④ rhs_segment_river_flux
  ⑤ rhs_river_downflow
  ⑥ rhs_deterministic_gather（替代 PassValue）
  ⑦ rhs_apply_dy
  每个块右侧注明并行策略：①②③④⑤ 可 parallel for，⑥ owner-local gather，⑦ parallel for。
  风格：模块化架构图，白底，块状。 -->
...
<!-- IMG: fig09_compute_gather_split.png -->
<!-- 生图提示：左右对比图，标题"S3 改造前后对比"。
  左侧"改造前"：fun_Seg_surface() 一个函数块内同时标注"计算 QsegSurf[i]"和"累加 QrivSurf[ir]+="，
  用红色标记累加部分为"共享写 / data race 风险"。
  右侧"改造后"：两个独立块：
  块 A "Pure Compute"（绿色）：每个 segment 只写 QsegSurf[i]，可并行；
  块 B "Owner-local Gather"（蓝色）：每个 river owner 按固定顺序从 QsegSurf 累加到 QrivSurf[ir]，可并行。
  两块之间用虚线隔开，标注"barrier"。
  风格：对比架构图，白底。 -->
...
<!-- IMG: fig10_parallel_phases_detail.png -->
<!-- 生图提示：P1–P7 的逐步展开图，展示 RHS core 的七个模块中哪些在哪个阶段被并行化。
  纵轴是 RHS core 的七个模块（同 fig08），横轴是 P1–P7 七个阶段。
  用绿色填充表示"该阶段并行化了该模块"，灰色表示"仍串行"。
  最终 P7 列全绿（全部并行），P1 列只有 reset/update 绿色。
  右侧标注精度要求：P1–P6 = A2（RHS bitwise），P7 = A3（full run bitwise）。
  风格：热力图/甘特图混合，白底。 -->
