...
```
阶段一：预并行（S0–S6）
  在还没动并行之前，先把单线程代码整理干净。
  → 产出：B1（parallel-ready 单线程参考结果）

阶段二：strict 并行（P1–P7）
  在 B1 上逐步打开 OpenMP，要求结果与 B1 逐 bit 一致。
  → 产出：P-strict（证明"并行没有改掉模型"）

阶段三：production 并行（P8–P9）
  放开 CVODE 内部向量并行和线性求解器优化，追求最大性能。
  → 产出：P-prod（在工程容差内的最快版本）
```
