#!/bin/bash
# tools/p8pre/render_n8_profile.sh
#
# p8pre-spike Step 1 PR-A (issue #339/#340) — wrapper that materializes the
# 18-cell N=8 Mode C profile recheck matrix by substituting markers in
# tools/p8pre/submit_n8_profile_template.sbatch and writing the rendered
# files to /scratch/frd_muziyao/SHUD-OpenMP/.p8pre-runs/rendered/.
#
...
# instead of auto-submitting. The PR-A run scope (issue #341) is responsible
...
#   /scratch/frd_muziyao/SHUD-OpenMP/.p8pre-runs/rendered/
...
#      wrapper merely renders; the runner script in #341 PR-A run scope
...
TEMPLATE="$SCRIPT_DIR/submit_n8_profile_template.sbatch"
...
OUT_DIR="/scratch/frd_muziyao/SHUD-OpenMP/.p8pre-runs/rendered"
...
# --dependency=afterany:<prev_jobid_placeholder> so the actual #341 PR-A
...
for case in "${CASES[@]}"; do
    ...
    for n in "${NS[@]}"; do
        ...
        for rep in "${REPS[@]}"; do
            ...
            singleton_name="p8pre_${case}_N${n}_singleton"
...
echo "[render_n8_profile] rendered $CELL_COUNT cells; write_rendered=$WRITE_RENDERED; out_dir=$OUT_DIR" >&2
