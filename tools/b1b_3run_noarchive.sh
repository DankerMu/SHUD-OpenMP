#!/usr/bin/env bash
# tools/b1b_3run_noarchive.sh
#
# B1b 3-run REPRODUCIBILITY validator (issue #188 S6c-12a).
#
# *** B1b VALIDATION MODE — ARCHIVE SECTION DISABLED ***
#
# Cloned from tools/archive_b0_output.sh on 2026-06-22 to refresh the qhh
# canonical summary SHA in docs/b1b_summary.md without touching
# benchmarks/<case>/B0_output/ (the B0 archive is immutable B0-tag golden).
#
# Divergence point: the archive-writing block at L442+ is gated by a hard
# `exit 0` immediately AFTER the 3-run identity verification block.
# DO NOT REMOVE THAT exit WITHOUT explicit gating logic — removing it would
# allow this script to OVERWRITE benchmarks/<case>/B0_output/ with B1b outputs,
# silently corrupting the B0-tag golden reference.
#
# Use:
#     OMP_NUM_THREADS=1 NUM_OPENMP=1 bash tools/b1b_3run_noarchive.sh <case> 3
#
# === ORIGINAL HEADER (for context) ===
#
# Original tools/archive_b0_output.sh purpose:
...
echo "[b1b_3run] all ${NUM_RUNS} runs SHA256-identical; SKIP ARCHIVE (B1b validation mode)"
echo "[b1b_3run] summary SHAs:"
for i in $(seq 1 "$NUM_RUNS"); do echo "    run${i}: ${SUM_HASHES[i-1]}"; done
echo "[b1b_3run] canonical summary SHA = ${SUM_HASHES[0]}"
echo "[b1b_3run] hash manifest contents (run 1):"
cat "${HASH_FILES[0]}"
exit 0

# === ORIGINAL ARCHIVE SECTION FOLLOWS — DISABLED ===
mkdir -p "$ARCHIVE_DIR"
