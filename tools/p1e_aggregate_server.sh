#!/usr/bin/env bash
# tools/p1e_aggregate_server.sh
#
# P1e PR-D (issue #312) — server 48-cell evidence aggregator.
#
...
# docs/p1e/p1e_pr_d_2x2_server.md.
#
# Input layout (default --runs /tmp/p1e_pr_d_server_runs):
#   <runs>/<build><case>_N<N>_rep<rep>/
#     cvode_stats.txt
...
#     cell.meta
#
# Cell naming on server is concatenated `Aheihe_N1_rep1` (no underscore
# between build and case), unlike the Mac PR-C layout `A_keliya_N1_rep1`.
# This script handles the concatenated form.
#
# Builds: A B   Cases: heihe heihe_x4   N: 1 2 4 8   reps: 1 2 3
...
#
# Usage:
#   tools/p1e_aggregate_server.sh [--runs <dir>] [mode]
#   tools/p1e_aggregate_server.sh all > docs/p1e/p1e_pr_d_2x2_server.partial.md

set -euo pipefail

RUNS="/tmp/p1e_pr_d_server_runs"
MODE="all"
...
emit_ac5() {
    ...
    echo
    echo "Mode B intra-case speedup (B N=1 / B N=k) per case:"
    echo
    echo "| case | N=2/N=1 | N=4/N=1 | N=8/N=1 |"
    echo "|------|--------:|--------:|--------:|"
    for c in "${CASES[@]}"; do
        local b1=$(mean_wall B "$c" 1)
        local row="| $c |"
        for n in 2 4 8; do
            local bn=$(mean_wall B "$c" "$n")
            local sp="?"
            if [[ "$b1" =~ ^[0-9.]+$ && "$bn" =~ ^[0-9.]+$ && "$bn" != "0" ]]; then
                sp=$(awk -v a="$b1" -v b="$bn" 'BEGIN{printf "%.3fx", a/b}')
            fi
            row="$row $sp |"
        done
        echo "$row"
