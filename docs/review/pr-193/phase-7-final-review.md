...
- Downstream #178 (S5d.1) compatibility: S5d.1 scope = hot-field SoA extraction on `_Element` AoS members + new `docs/b1b/s5d_hot_fields.yaml`. S5b manifest documents scratch array writers (Model_Data `double*` arrays, not `_Element` members). Jagged scratch flattening deferred to #179 S5d.2-5a per #178 body. No conflict.
