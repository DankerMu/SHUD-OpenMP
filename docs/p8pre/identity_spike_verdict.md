...
The ratio `nfeLS / nfe = 12120 / 6696 = 1.811` confirms substantial linear-solver overhead. A productive preconditioner would: (a) reduce `nli` (currently 12120) per Newton step, (b) eliminate `ncfn` floor by stabilizing Newton, (c) reduce `ncfl` SPGMR restarts. Identity preconditioner achieves none of these — establishing the operational ROI ceiling. **PASS criterion for any future P8-precond candidate: `ncfn_candidate ≤ 7 (heihe) ∧ ncfn_candidate ≤ 51 (heihe_x4)` (per `docs/p8pre/n8_profile_verdict.md` §3.1 cleaned-PREC_NONE baseline) with combined setup-plus-solve overhead within 10% of identity-baseline wall. These 6/47 are deterministic floors observed for `PREC_LEFT + identity`, a negative-control architecture, NOT for the production `PREC_NONE` codepath.**
...
**Cleanup status update (post-merge 2026-06-27)**: items 2-3 above (revert `cvode_config.cpp:259` + delete `MD_precond_identity.{h,cpp}` + unlink Makefile + remove `CVodeSetPreconditioner` / `CVodeSetLSetupFrequency(50)` registrations) are COMPLETED at outer `e442ce8` / SHUD `37be0fe` via the subsequent cleanup pointer-bump PR. Item 4 (Timer bucket) and item 5 (close `baseline/p8pre`) remain pending non-blocking actions, kept open for future preconditioner-candidate reuse. The identity spike artifact (`MD_precond_identity.{h,cpp}` jok-mirror PSetup + memcpy PSolve canonical pattern) is preserved in git history at SHUD `5276167` for future reference by any P8-precond candidate epic re-fork.

**Forward carry to future P8-precond / P8-tune candidates** (when/if intaken):
...
- The 1.811 `nfeLS/nfe` ratio and the cleaned-PREC_NONE `ncfn = 7 (heihe) / 51 (heihe_x4)` floors (per `docs/p8pre/n8_profile_verdict.md` §3.1) form the ROI baseline against which Diagonal/Jacobi/ILU(0)/SPGMR-maxl candidates must demonstrate improvement. The PREC_LEFT-identity floor 6/47 is retained ONLY as negative-control anchor.
...
- The first P8-tune.C probe (SPGMR `maxl` sweep) is being designed in capability [`spgmr-maxl-env-hook`](../../openspec/changes/p8tune-spgmr-maxl/specs/spgmr-maxl-env-hook/spec.md) + [`maxl-sweep-verdict`](../../openspec/changes/p8tune-spgmr-maxl/specs/maxl-sweep-verdict/spec.md); ADR-0004 (forthcoming) is the verdict gate.
...
- **Prerequisite 1**: Real preconditioner candidate (Diagonal/Jacobi/ILU(0)) must demonstrate `ncfn_candidate ≤ 7` (heihe) AND `ncfn_candidate ≤ 51` (heihe_x4) at acceptable setup cost (per `docs/p8pre/n8_profile_verdict.md` §3.1 cleaned-PREC_NONE baseline). The `PREC_LEFT + identity` floor 6/47 is retained ONLY as negative-control anchor, NOT as the production PASS gate. Identity stub establishes the no-rotation baseline.
