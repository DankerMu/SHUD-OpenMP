...
## Core determinism argument (single source; design.md repeats verbatim)
...
| E2 (Tier 2, conditional) | StrictOMP | OpenMP element-wise + fixed-tree reductions | bitwise cross-N; one-time shift vs C → re-baseline + A5 |
...
~µs per op at NY≈120k). Deliverable: per-op-class share table —
element-wise vs reduction vs non-NVector remainder of `t_CVODE_raw` — on
heihe_x4 (90-day, server Slurm) + keliya (sanity, Mac) + heihe_x16 short
window (share convergence check). Default path untouched (gate off = stock
behavior; bitwise gate evidence per P11-osc precedent).

## Kill / ROI gates (quantified; single source for spec + verdict doc)
...
- **G-E2 (Tier-1 ROI)**: heihe_x4 server matrix N∈{1,8,16}, Config E vs Config C:
  ADOPT if wall improvement ≥ 1.10× at N=8 or N=16 with A3a bitwise PASS
  (expected: identical trajectories → identical nst/nfe; wall-only comparison);
  else TIER1_CLOSE (document, keep code默认关闭 or revert per verdict).
- **G-E3 (Tier-2 GO)**: ALL of — (i) Tier 1 ADOPTED; (ii) PR-N0 profile shows
  reduction-op share ≥ 10% of t_CVODE_raw at heihe_x4 or x16 (legacy
  P8-NVector.2 rule); (iii) Amdahl projection from measured shares gives
  ≥ 1.15× additional wall over adopted Config E at N=16. Any one fails →
  TIER2_NO-GO (closure note; fixed-tree code NOT written).
- **G-E4 (Tier-2 acceptance, only on GO)**: cross-thread bitwise (N∈{1,2,4,8}
  identical) + A4 max_ulp vs Config E reference documented + full A5 hydrology
  acceptance (NSE/KGE ≥ 0.99, peak ≤ 1 interval, runoff ≤ 1%, water balance
  non-degrading — tools/a5 pipeline) + explicit new-golden re-baseline decision
  recorded (mode-C-tune precedent: A3a does not apply across the reduction-order
  change; applies within). Fail → revert to Config E.
...
- **PR-N0** (SHUD + outer evidence): `SHUD_NVEC_PROF=1` ops-wrapper profiler +
  bitwise-neutrality evidence + op-share tables (x4/x16/keliya) + share verdict.
...
- **PR-N2** (outer): server Slurm scaling matrix Config E vs C + G-E2 verdict +
  ADR-0011 + master plan §P12-nvec anchor + glossary §P12-nvec 集合 + Tier-2
  gate decision (G-E3) recorded.
- **PR-N3** (CONDITIONAL on G-E3 GO; SHUD + outer): fixed-tree deterministic
  reductions (block size fixed; Neumaier compensation optional per legacy §P9
  menu) + G-E4 acceptance + re-baseline decision doc.
