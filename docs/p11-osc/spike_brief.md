...
(kashigeer B0 archive lacks `cvode_stats.txt` — refresh opportunistically.)
...
- Interval-mean dt = SolverStep / delta_nst localizes collapse in model time.
- Env unset → zero file I/O, zero extra calls (whole block behind the gate);
  default binary path untouched. Gate proof: keliya B0 bitwise SHA pre/post.
- Precedent: `SHUD_DUMP_CV_Y` env-gated dump (P1e PR-B, shud.cpp:262).
...
diff per-element states (`uYsf`, `uYus`, `uYgw`, river stages) against the
previous interval and count sign alternations of the interval delta
(rise→fall→rise = 1 flip). O(NumEle) compare + one state copy per interval.

Output: per-element cumulative flip counts + per-day windowed counts (CSV at
run end). Oscillating cell pairs see-saw water → their storage deltas
alternate sign at high frequency; real monotone drainage/recharge does not.

### D3. Analyzer (outer `tools/osc_diag/`, Python uv project)

Consumes D1+D2 CSVs, emits:

- dt histogram + **burst share**: fraction of total nst spent in intervals
  with mean-dt below thresholds {60 s, 10 s} — the speedup ceiling estimator
  (wall ∝ nst; eliminating bursts caps gain at 1/(1−burst_share)).
- top-K flip elements + spatial concentration (what % of flips live in top 1%
  of elements) + adjacency clustering (are they neighbor pairs?).
- temporal cross-check: do flip-count spikes coincide with dt minima?
- `MARKER:OSC_DIAG_VERDICT` block: verdict + all gate inputs, machine-readable.

### Case matrix (all 90-day, local Mac, serial + Config C)
...
| heihe | healthy control (6.5k steps) | ~3.5 min |

No Slurm required; iteration loop is entirely local. 90-day iron rule holds.

## Kill-gate (quantified, evaluated by D3 verdict)

**OSC_CONFIRMED** requires ALL of:
1. burst share ≥ 50% of nst at mean-dt < 60 s on qinyijiang (ceiling ≥ 2×,
   i.e. beats the closed parallel line before any limiter work is justified);
2. top 1% of elements carry ≥ 50% of sign flips (localized culprits, not
   basin-wide behavior);
3. flip spikes temporally coincide with dt minima (mechanistic link).

**REAL_DYNAMICS** (any of): flips distributed ~uniformly; flip spikes track
forcing peaks instead of dt minima; burst share < 50%.
→ write closure note, close epic, no limiter work.

**INCONCLUSIVE** → at most ONE follow-up diagnostic refinement (e.g. per-edge
flux instrumentation on top-K elements), then forced re-verdict. No limiter
work on an inconclusive verdict.
...
- heihe/heihe_x4 need no treatment — they are healthy controls.

## Branch / merge model (user-approved 2026-07-01)

- Outer work → `research/p11-osc`, PRs target `main` per normal flow.
