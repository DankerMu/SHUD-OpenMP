...
Two ratio interpretations emitted：

**(A) spec L148-149 LITERAL**: `sizeof(ElementHotData) / sizeof(_Element)`

| 平台 | sizeof(ElementHotData) | sizeof(_Element) | ratio (A) | 阈值 | verdict |
|---|---|---|---|---|---|
| Apple Silicon macOS (Apple clang 17) | 256 B | 688 B | 0.3721 | < 0.20 | **FAIL** (IN-PROGRESS) |
| Linux x86_64 cn05 (Ubuntu 24.04 GCC 13.3, Slurm 8621) | 256 B | 688 B | 0.3721 | < 0.20 | **FAIL** (IN-PROGRESS) |

**(B) master plan §S5d L1432 cache-meaningful**: per_ele_soa_bytes / sizeof(_Element)

| 平台 | per_ele_soa_bytes | sizeof(_Element) | ratio (B) | 阈值 | verdict |
|---|---|---|---|---|---|
| Apple Silicon macOS | 300 B | 688 B | 0.4360 | < 0.20 | **FAIL** (IN-PROGRESS) |
| Linux x86_64 cn05 (Slurm 8621) | 300 B | 688 B | 0.4360 | < 0.20 | **FAIL** (IN-PROGRESS) |

跨平台两端数字 **完全一致** — 是 layout 决策驱动,非 ABI / 编译器差异。
...
**Fallback**: wall-clock comparison（B1a-tag vs B1b heihe 90 天 NUM_OPENMP=1 OMP_PROC_BIND=close 3 run 中位数）。Wall-clock 是 cache miss 的间接指标——cache miss ↓ 30% 通常对应 wall-clock ↓ 5-15%（视 RHS 在 t_total 占比而定）。
...
| spec target ≤ 0.87 (>= +15% speedup) | spec L155-157 | **IN-PROGRESS** (1.0000 > 0.87) |
...
**对 #184 (S6b.1) 的影响**: 0。S5d.3 first-touch 基础设施已在位,S5d.4 thread binding wrapper 已在位,等 P1+ RHS parallel-for 落地后 NUMA accel 可重新测。spec L153 显式允许 8.3 IN-PROGRESS 不阻 #184(design R4 mitigation #2 "拆开 B1b 内部 critical path")。
...
| cn 双 socket node + Apple Silicon 各 cache miss ↓ ≥ 30% (T8.2, scenario "cache miss 单线程下降 ≥ 30%") | **IN-PROGRESS** (server `perf_event_paranoid=4` blocks HW counters; wall-clock proxy: cn07 B1b/B1a = 0.9980 / Mac 0.9819 — noise-floor signal) |
| NUMA accel ratio ON/OFF ≤ 0.87 (T8.3, scenario "双 socket NUMA 加速比") | **IN-PROGRESS** (cn07 8 thread ON/OFF = 1.0000 — B1b serial-only baseline 客观反映; spec L153 + R4 mitigation #2 显式允许不阻 #184) |
...
- Server perf stat HW counters: 需要 cluster admin 把 `perf_event_paranoid` 改 `<= 2` 才能在 sbatch 内 access。属于基础设施改动,不在本 PR 范围。S6c capstone 前可尝试 escalate。
- Apple Silicon perf stat: `xctrace` 报告格式与 Linux `perf stat` 不兼容,跨平台比较无意义。可作 BONUS local CI gate(不进 gating)。
- 跨平台 sizeof 数字: Linux x86_64 GCC 11 实测可能与 macOS Apple clang 17 略有差异(ABI padding),T8.1 gate 在两端独立 emit。
