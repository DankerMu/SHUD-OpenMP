...
**CI 不承载 bitwise / CVODE / snapshot 90d gate（post-cleanup PR 彻底删 data_probe scaffold）**：早期 `.github/workflows/serial-baseline.yml` 曾持有 cfg.para 动态 awk 解析 START 解析 + snapshot HARD-fail gate + bitwise compare vs B0-tag golden 步骤（PR #54 + #57 dynamic-START parse）。但因 forcing data 不在 CI runner，data_probe 永远返回 `data_available=false`，30+ run+compare step silent skip with notice = dead code。**follow-up cleanup PR 已按 KISS/YAGNI 彻底删除 data_probe + 30 if-guarded scaffold**。当前 CI 5 类 active gate（全 data-independent、每 PR 必跑）：3-Config Makefile build + 4 macro grep + invariant-sweep + B0-tag smoke + skip-label deletion verify + check_goldens 48 SHA 漂移检查。bitwise / CVODE 15-key / snapshot 90d gate 全部依赖本地 + 服务器人工验证（4-case × 90-day 在 S1 已 16+ 次跑过 PASS，详见 docs/s1_summary.md "下一步：S2" §3）。**未来如要让 CI 真跑 bitwise**：cleanup PR parent commit (`ceb02d1`) 有完整 scaffold 实现，git history cherry-pick 即可。matrix Config A/B/E × keliya = 3 jobs（post-#103 expand：Config A 默认 / Config B `SHUD_LEGACY_OMP_RHS=1` legacy `_omp` TU + nm 3 符号 gate / Config E `SHUD_ENABLE_OPENMP_RHS=1` P7 strict build-only PASS；nightly cron + full-bitwise label 仍由 post-#74 删除）。
