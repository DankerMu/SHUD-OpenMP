...
> _最近一次更新：2026-06-20（#133 SoA refactor via this PR：Mac A3a 4/4 PASS + Config A neutrality + nm gate PASS；server wallclock ≥ 1.5x acceptance 走 post-merge sequel；#134 server-gcc13.x86_64 anchor 4 张 closed via PR #139；S2 P7 acceptance gates closed：#126 A3a/A3b PASS, #127 wallclock heihe PASS + heihe_x4 PARTIAL → P8 sibling #133 进 progress；A3a server FAIL = anchor 平台错配，#134 closed 后解除；Mac 4/4 PASS 反证 fusion correctness 完整；P7 epic #100 closed; 后续 #135 + #137 closed via PR #136 + #138 — 余下 #123 需服务器执行）_
...
| [#133](https://github.com/DankerMu/SHUD-OpenMP/issues/133) | heihe_x4 wall-clock 1.04x → 1.5x via memory-layout (SoA + cache-block + NUMA + simd) | heihe_x4 1.5x wall-clock 完整闭合 | P1 | **PARTIAL** — SoA refactor done via this PR (Mac A3a 4/4 PASS, Config A neutrality PASS); server wallclock ≥ 1.5x acceptance 走 post-merge sequel |
