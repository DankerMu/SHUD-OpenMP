...
> _最近一次更新：2026-06-28（P8 sub-epoch chronology backfill：**p8tune-klu-spike epic staged** [#379 + #380-383 sub-issues] via stage-change-pipeline workflow (Stage 3 三路审核 1 round clean / 28 finding 全 resolved / gate_net_catch=0 / Stage 5.5 align clean)，覆盖 FD colored Jacobian + 5-block adjacency + KLU symbolic/numeric wrapper + 16-cell Slurm sweep + 3-axis verdict + ADR-0005 4-branch (GO/Optional/Case-aware/NO-GO with BoomerAMG retreat) — 4-PR scope (PR-0 tool / PR-A sweep / PR-B aggregator+ADR / PR-C capstone), 2-3 周 budget, pattern-only (zero SHUD source patch / zero CVODE wire-up / zero model run); decisive cell heihe_x4 NumY ~120K; 主 plan §P8-tune.D anchor + ADR-0004 §Forward action 已 commit。**P8-tune.D 上一前置 (2026-06-28)**：p8tune-doc-correction epic capstone (PR #378 merged) — NumY 工作集口径 corrected from N_elem to NumY=3·NumEle+NumRiv+NumLake per `SHUD/src/Model/shud.cpp:139` (heihe ~19K → 1.52 MB / heihe_x4 ~120K → 9.6 MB / heihe_x16 ~760K → 60 MB), `SHUD_SPGMR_MAXL=30` wording from RECOMMENDED → **Performance opt-in (NOT A5-certified)**, master plan §P8-tune.C [CLOSE 2026-06-28] + §P8-tune.D KLU spike anchor 新建 (60+ lines)。**p8tune-spgmr-maxl epic capstone (2026-06-27, p8tune-tag locked baseline/p8tune HEAD c8b2ef7)**：6-PR sequence (PR-0 doc-correction → PR-A clean-PREC_NONE-baseline → PR-B verdict-gate → PR-C SHUD_SPGMR_MAXL env-hook → PR-D 60-cell sweep → PR-E aggregator+ADR-0004); ADR-0004 verdict = **Optional-knob branch** (heihe N=1 maxl=30 = +11.99% wall GO band; heihe_x4 全 maxl ≥10 wall REGRESS -6.86%~-24.82%, SPGMR-tune 对 production scale saturated)。**p8pre-spike epic capstone (2026-06-27, p8pre-tag locked baseline/p8pre HEAD e442ce8)**：7-PR sequence (Step 0 doc-correction + PR-A/B/C N=8 profile recheck Mode C + PR-D/E/F identity precond stub wire-up + PR-G archive); ADR-0003 verdict = **NO-GO option (b)** (H2 hard FAIL: identity P⁻¹=I 对 SHUD stiff Jacobian Newton 收敛零 ROI / S5 soft FAIL: PREC_LEFT 状态机 inherent cost ≈9×10¹⁵ ULP drift > A4 1024 threshold), design D8 fall-back PR 还原 PREC_NONE (SHUD pin `37be0fe`); D11 chain 8-tag → 10-tag (P1e → p8pre → p8tune)。**P1e epic capstone (2026-06-25)** via §4.6.2 partial-closure → SHIP：PR-A #309 / PR-B #310 / PR-B0 #311 / PR-C #312 / PR-D #313 / PR-E #314 / PR-F + PR-G #315 / PR-H #316 / PR-I #317 / PR-J #318 + #333 / PR-K 本 PR / PR-L 下一 PR / PR-M 待 PROMOTE；SHUD pin `210ac191...` → `3341368d2d0854924d2286925c8575df52cc97a0` (PR-F StrictOMP impl + PR-G -fopenmp wire + SHUD_RHS_THREADS env + PR-H first-touch removal + omp single→omp for; openmp-baseline pushed, master untouched)；3 SHALL gate verdict = AC-S1 PASS (mode C 跨 N bitwise heihe `a2023ccd2de4` + heihe_x4 `b5e4b0a2cf83`) + AC-S2 PASS (6-case mode C SHA == mode A reference SHA roll-up) + AC-S3 PARTIAL (heihe sp@8 1.066× < 1.3× FAIL, heihe_x4 sp@8 1.729× ≥ 1.5× PASS; AND-gate 不满足 → §4.6.2 partial-closure → user 决策 SHIP); 4-mode `strict-omp` 列状态 "production candidate (P1e 验收前)" → "SHIP via §4.6.2 partial-closure (P1e 验收后)"; ADR-0002 Status `Accepted` → `Implemented (P1e epic close, 2026-06-25)`; D11 6-tag chain → 7-tag chain (P1e-tag 新增, 待 PR-L push); 详 [`docs/p1e/p1e_summary.md`](p1e/p1e_summary.md) §1-§10 + [`docs/p1e/p1e_2x2_verdict.md`](p1e/p1e_2x2_verdict.md) §6 + [`docs/p1e/p1e_pr_i_strict_omp_verification.md`](p1e/p1e_pr_i_strict_omp_verification.md) + [`docs/p1e/p1e_perf_baseline.md`](p1e/p1e_perf_baseline.md) + [`docs/p1e/p1e_report.md`](p1e/p1e_report.md) §1-§10 + master plan v1.5 §6 P1e。**P1d epic 上一次更新 (2026-06-24)**：P1d epic capstone via E′ containment path：PR-C0 #292 / PR-C #293 / PR-D #294 / PR-E #295 / PR-F #296 / PR-G #297 / PR-H #298 / PR-I #299 / PR-K #300 / PR-L #301 / PR-M #302；SHUD pin 3a0004c → 210ac19 (PR-G Kahan revert + first-touch stacked, openmp-baseline pushed, master untouched)；outer HEAD `a19fb5e` (master plan v1.5 / M10 merged)；3 SHALL gate verdict = FAIL at N≥4 (A3a + nst Δ FAIL; N=1 reverse-compat PARTIAL PASS); 用户决策走 **E′ containment closure**: 4-mode spec rewrite (`serial` / `strict-omp` / `det-omp` / `fast-omp`) + PR-C/D/E steady-state first-touch 标 DEPRECATED + PR-G Kahan revert 保留 + production 默认 NUM_OPENMP=1 + `shud_omp` 标 `fast-omp experimental, non-production`; 真正根因 = SUNDIALS 6.0.0 `NVECTOR_OPENMP` reduction tree 跨 N 不固定 (5/5 codebase fact-check confirmed: f.cpp:54 always Serial RHS + MD_rhs_core.cpp:802-811 StrictOMP/ProductionOMP abort 桩 + Makefile shud_omp 硬编码 NVECTOR_OPENMP + SPGMR 无 preconditioner); 详 [`docs/p1d/p1d_summary.md`](p1d/p1d_summary.md) + [`docs/p1d/p1d_numa_root_cause.md`](p1d/p1d_numa_root_cause.md) + [`docs/p1d/p1d_perf_baseline.md`](p1d/p1d_perf_baseline.md) + [`docs/p1d/p1d_report.md`](p1d/p1d_report.md) + [`docs/p1d/p1d_pr_h_final_run.md`](p1d/p1d_pr_h_final_run.md) + master plan v1.5 §6 P1d / P1e。**P1c epic 上一次更新 (2026-06-23)**：P1c-tag deref baseline/P1c HEAD / SHUD pin `3a0004c` (Kahan-injected, openmp-baseline pushed)；8-site canonical-reduction Requirement CLOSED via helper-wrap；§4.4 A3a + §4.5 nst Δ=0 PARTIAL CLOSURE + carve-out 推 P1d (per master plan §3 fallback option 2 + spec L100-L103)。**P1 epic 上一次更新 (2026-06-22 earlier)**：P1-update-omp-tag `ff21c75c` deref `003f58d` / SHUD pin `07c677f`；`baseline/P1` lock_branch=true 已锁；详 [`docs/p1_summary.md`](p1_summary.md) + [`docs/p1/p1_rhs_snapshot_bitwise.md`](p1_rhs_snapshot_bitwise.md) + [`docs/p1/p1_fullrun_bitwise.md`](p1_fullrun_bitwise.md) + [`docs/p1/p1_perf_baseline.md`](p1_perf_baseline.md)。_
...
| **P1c**   | PARTIAL @ Mac (pre-Kahan only) | PARTIAL @ Mac (pre-Kahan only) | PARTIAL @ Mac (pre-Kahan only) | N/A (deferred-upstream) | PARTIAL @ Mac (pre-Kahan only) | PARTIAL @ server | PARTIAL @ server | **PARTIAL CLOSURE** (8-site canonical-reduction Requirement CLOSED via PR-B/C/D/E helper-wrap + Kahan held-in-reserve PR-G applied at PR-I; bit-level A3a + nst Δ=0 cross-N carved-out 推 P1d (per master plan §3 fallback option 2 + spec L100-L103); NUM_OPENMP=1 reverse-compat 在 Kahan-injected HEAD FAIL/D11 tag-immutability 保留; Mac 4-case 16-cell scan documented PR-F 但 P1-update-omp-tag Mac canonical rivqdown.dat SHA reference NOT directly available in P1 era docs → spec L154-157 Scenario DEFERRED to P1d + retroactive reference capture — per `docs/p1c_summary.md` §1 完成定义 + §5.2 P1d hand-off + §6.8 Mac SHALL Scenario deferred note + §8 反向兼容判定) |
| **P1d**   | PARTIAL (Mac PR-I anchor archived) | PENDING (P1e Mac matrix) | PENDING (P1e Mac matrix) | N/A (deferred-upstream) | PARTIAL (Mac PR-I anchor archived) | PARTIAL @ server (PR-H final) | PARTIAL @ server (PR-H final) | **PARTIAL CLOSURE via E′ containment path** (NUMA env + first-touch + Kahan revert preserved; 3 SHALL gate FAIL at N≥4 — A3a + nst Δ FAIL; N=1 reverse-compat PARTIAL PASS for server heihe; PR-C/D/E steady-state first-touch loops 标 DEPRECATED — real RHS still Serial per fact-check #1+#2; 4-mode spec rewrite — `serial` / `strict-omp` (P1e) / `det-omp` / `fast-omp`; production default `NUM_OPENMP=1`; `shud_omp` 标 `fast-omp experimental, non-production`; root cause = SUNDIALS NVECTOR_OPENMP `reduction(+:sum) schedule(static)` 跨 N tree shape 不固定 — 不是 first-touch / NUMA / SPGMR precond; rivqdown.dat mean_rel 10-25% at N≥4 — cross gauge precision; P1e (F 路) = Serial N_Vector + StrictOMP RHS 是下个 epic — 详 [`docs/p1d/p1d_summary.md`](p1d/p1d_summary.md) §1-§12 + [`docs/p1d/p1d_numa_root_cause.md`](p1d/p1d_numa_root_cause.md) §1-§13 + [`docs/p1d/p1d_report.md`](p1d/p1d_report.md) §1-§12 + master plan v1.5 §6 P1d) |
| **P1e**   | PASS @ Mac (PR-J N=1 mode C SHA == mode A reference) | PASS @ Mac (PR-J N=1) | PASS @ Mac (PR-J N=1) | N/A (deferred-upstream) | PASS @ Mac (PR-J N=1) | PARTIAL @ server (PR-I 4 N: AC-S1+S2 PASS, AC-S3 sp@8 1.066× < 1.3× FAIL) | PASS @ server (PR-I 4 N: AC-S1+S2 PASS, AC-S3 sp@8 1.729× ≥ 1.5× PASS) | **SHIP-LOCKED via §4.6.2 partial-closure** (P1e-tag tag-object `25023eff32d1fa317b045cbc786f379fac9e522c` deref `11687b756dd53bb634df391bcbeb64b3cef5c750`, PROMOTE date 2026-06-25 via PR-M; baseline/P1e re-lock by orchestrator post-merge; 3 SHALL gate: AC-S1 mode C 跨 N bitwise PASS heihe `a2023ccd2de4` + heihe_x4 `b5e4b0a2cf83`; AC-S2 6-case mode C SHA == mode A reference SHA roll-up PASS; AC-S3 D7 PARTIAL — heihe FAIL sp@8 1.066× < 1.3× threshold + heihe_x4 PASS sp@8 1.729× ≥ 1.5× threshold; AND-gate (BOTH FAIL 才触发 D12.3) 不满足 → §4.6.2 partial-closure → user 决策 SHIP per docs/p1e/p1e_2x2_verdict.md §6.3-§6.4 SHIP rationale + heihe small-case OMP overhead floor 设计预期 carve-out; D12 4 branch eval = D12.1/.2/.3/.4 全 NOT triggered (placeholder p1e_pr_n_block_jacobi.md 写出); SHUD pin `3341368d` (PR-F StrictOMP impl + PR-G -fopenmp wire + SHUD_RHS_THREADS env + PR-H first-touch removal); production default 转 `SHUD_RHS_THREADS` per case (heihe `=1` carve-out / heihe_x4 `=4` 推荐); 4-mode `strict-omp` 列状态 "SHIP via §4.6.2 partial-closure (P1e 验收后)"; ADR-0002 Status `Implemented (P1e epic close, 2026-06-25)`; nst Δ=0 跨 N strict closure (mode B era 跨 N drift → mode C 闭合); 详 [`docs/p1e/p1e_summary.md`](p1e/p1e_summary.md) §1-§10 + [`docs/p1e/p1e_2x2_verdict.md`](p1e/p1e_2x2_verdict.md) §6 + [`docs/p1e/p1e_pr_i_strict_omp_verification.md`](p1e/p1e_pr_i_strict_omp_verification.md) §3-§8 + [`docs/p1e/p1e_mac_reverse_compat.md`](p1e/p1e_mac_reverse_compat.md) §3.5 6-case roll-up + [`docs/p1e/p1e_perf_baseline.md`](p1e/p1e_perf_baseline.md) §3-§6 + [`docs/p1e/p1e_strict_omp_design.md`](p1e/p1e_strict_omp_design.md) + [`docs/p1e/p1e_report.md`](p1e/p1e_report.md) §1-§10 + master plan v1.5 §6 P1e) |
| **P2a**   | PENDING| PENDING             | PENDING    | PENDING   | PENDING | PENDING       | PENDING       | PENDING (启动前置改链由 P1d → P1e 更新: 新前置 = P1e (F 路) `strict-omp` mode 完成 + 3 SHALL gate (AC-S1+S2 PASS + AC-S3 partial-closure SHIP) + heihe_x4 sp@8 ≥1.5× 闭合 (1.729×) + `P1e-tag` push + `baseline/P1e` lock_branch=true + ADR-0002 Status `Implemented (P1e epic close)` + OpenSpec p1e-strict-omp-rhs + p1e-capstone 2 spec PROMOTE + glossary 4 new terms 入册; 见 docs/p1e/p1e_summary.md §9 forward handoff + ADR-0002 §"Implementation closure")   |
...
- ship caveats — 全部 RESOLVED 通过 PR-19 #210 升级为 **UNCONDITIONAL ship**（详 `docs/b1b_summary.md` §"B1b ship status" + `SHUD/B1b_CHANGELOG.md` S6b.2/S6b.4/S6b.5 + `docs/b1b/s217_lake_formula_audit.md` §E）：
...
- **P2a-P7** 各行按 master plan §3 各阶段填；P2a M12 状态 = NOT 启动 (per master plan §P2a NO-GO decision 2026-06-26); P2b ABSORBED by P1e PR-H (per master plan §P2b); P3-P7 PENDING。
- **P8** 矩阵主 row 仍 PENDING (master plan §3 P8 主阶段 = "CVODE 内部并行 / 求解器精度等级 P-prod" 整体阶段, 由后续 P8-tune.E full KLU + A5 OR P8-tune.F BoomerAMG/Hypre 实施 epic 收尾)；当前 P8 sub-epoch spike 进度 (post-P1e ship 起累计):
  - **P8-precond.0 / p8pre-spike**: epic close 2026-06-27, `p8pre-tag` 锁 `baseline/p8pre` HEAD `e442ce8`, ADR-0003 verdict = **NO-GO option (b)** (identity P⁻¹=I 不 unlock Newton 收敛 + PREC_LEFT 状态机 inherent ULP drift), design D8 fall-back PR 还原 PREC_NONE (SHUD pin `5276167` → `37be0fe`)。
  - **P8-tune.A/B/C / p8tune-spgmr-maxl**: epic close 2026-06-27, `p8tune-tag` 锁 `baseline/p8tune` HEAD `c8b2ef7`, ADR-0004 verdict = **Optional-knob branch** (`SHUD_SPGMR_MAXL=30` for heihe N=1 = +11.99% wall GO band, **Performance opt-in tier NOT A5-certified** per PR #378; heihe_x4 全 maxl ≥10 wall REGRESS -6.86%~-24.82%, SPGMR-tune path saturated for production scale)。
  - **p8tune-doc-correction**: epic close 2026-06-28 (PR #378 merged `97b88d5`), NumY 工作集口径 correction (N_elem → NumY=3·NumEle+NumRiv+NumLake per `SHUD/src/Model/shud.cpp:139`) + maxl=30 tier 降级 (RECOMMENDED → Performance opt-in) + master plan §P8-tune.C [CLOSE] + §P8-tune.D KLU spike anchor 新建。
  - **P8-tune.D / p8tune-klu-spike**: epic staged 2026-06-28 ([#379 Epic](https://github.com/DankerMu/SHUD-OpenMP/issues/379) + #380-#383 sub-issues), OpenSpec change 通过 stage-change-pipeline workflow (Stage 3→5.5 clean 1-round)。**[OPEN, executing PR-0]** state: 4-PR scope (PR-0 spike tool / PR-A 16-cell Slurm sweep / PR-B aggregator+ADR-0005 / PR-C capstone), 2-3 周 budget, 决定 KLU 作 SHUD large-case linear solver candidate 是 GO / Optional / Case-aware / NO-GO (NO-GO 退路 = P8-tune.F BoomerAMG/Hypre spike)；pattern-only (zero SHUD source patch / zero CVODE wire-up / zero model run / zero hydrology comparison)。
- **P9** PENDING (A5 hydrology-equivalence epic;触发条件 = `SHUD_SPGMR_MAXL=30` tier promote OR KLU/AMG candidate 进入 P8-tune.E 后)。
