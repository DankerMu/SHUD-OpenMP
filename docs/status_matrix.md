...
| **P1e**   | PENDING| PENDING             | PENDING    | PENDING   | PENDING | PENDING       | PENDING       | **PENDING (待启动)** (F 路: Serial N_Vector + StrictOMP RHS 替换 abort 桩; 启动前置 = 2×2 build matrix 因果实验 (A=Serial NVec+Serial RHS / B=OpenMP NVec+Serial RHS=current `shud_omp` / C=Serial NVec+StrictOMP RHS=候选 production / D=OpenMP NVec+StrictOMP RHS=research) × N∈{1,2,4,8} × 3 reps + hash `CV_Y` + rivqdown.dat + 15-key CVODE stats; 判据 C 跨 N bitwise + nst Δ=0 + 加速 ≥1.5× → F 路成立 → P1e.2-7 实施; fallback 至 ADR-0001 (NVECTOR_REPRO_OMP / block-Jacobi precond / KLU); 详 master plan v1.5 §6 P1e + [`docs/p1d/p1d_summary.md`](p1d/p1d_summary.md) §9 forward handoff + ADR-0001 (forthcoming) + openspec/changes/p1e-strict-omp-rhs (forthcoming) — Phase 2(e) 并行 agent owns) |
| **P2a**   | PENDING| PENDING             | PENDING    | PENDING   | PENDING | PENDING       | PENDING       | PENDING (启动前置改链: 原 P1c-tag + P1c.3 不再充分 → 新前置 = P1e (F 路) 完成 strict-omp mode 实现 + 3 SHALL gate strict-omp mode 通过 + 加速比 ≥1.5× + `P1e-tag` 已 push + `baseline/P1e` lock + ADR-0001 close out)   |
