...
| (b) 3 negative grep gates PASS | spec Scenarios L76/L81/L86: 新宏 0 / `schedule(dynamic\|guided)` 0 / `#pragma omp atomic` 0 | ✓ COMPLETE (per `docs/p1c_summary.md` §6.1) | PR-F |
| (c) 10-anchor coverage table 完整 | spec L63-L66 Scenario: 10 write target × 4 helper mapping | ✓ COMPLETE (per `docs/p1c_summary.md` §6.2) | PR-F |
| (d) Kahan held-in-reserve patch documented | spec L107-L128 Requirement: conditional Kahan + trigger + apply flow | ✓ COMPLETE (`docs/p1c_kahan_patch.diff` + a3a §"Kahan 候选路径") | PR-G |
| (e) Server PR-K2 首跑 (8-cell) executed + verdict | spec §4 + design D3 SHALL gate | ✓ COMPLETE → §4.7 trigger FIRED (A3a + nst 双 FAIL) | PR-H |
| (f) §4.7 Kahan injection 二跑 (8-cell) executed + verdict | spec L107-L128 conditional path | ✓ COMPLETE → PARTIAL CLOSURE + P9 carve-out | PR-I |
...
- ✗ Upstream parallel writer first-touch / NUMA-affinity 治理 (OMP_PROC_BIND/OMP_PLACES + numactl interleave) — P9 stage 独立范围
...
- **tag annotated message** (per PR-L 草拟):
  ...
  - PARTIAL CLOSURE: deterministic-reduction Requirement landed; bit-level A3a + nst Δ=0 carved-out to P9 (upstream writer first-touch / NUMA governance)
...
### §5.2 P9 carve-out (deferred stage)

P9 stage (per master plan §3 P9 row) 负责:
...
2. **Bit-level A3a cross-N closure**
   - 验证 P9 NUMA 治理后 heihe / heihe_x4 4 N SHA 全等
   - 若 PASS: Kahan injection 可 revert (`docs/p1c_kahan_patch.diff` 反向 apply)
...
5. **Mac reframing 二次验证**
   - Mac 16-cell scan post-NUMA-fix 应 PASS (per `docs/p1c_perf_baseline.md` §1.6 hypothesis)
...
详 `docs/p1c_pr_h_server_first_run.md` §3-§7.
...
详 `docs/p1c_pr_i_kahan_injection.md` §3-§7。
...
详 `docs/p1c_a3a_root_cause.md` §"design D9 decision branch 判定" 终判表。
...
详 `docs/p1c_pr_j_reverse_compat.md` §2-§4。
...
- PARTIAL CLOSURE: deterministic-reduction Requirement landed; bit-level A3a +
  nst Δ=0 carved-out to P9 (upstream writer first-touch / NUMA governance)
- NUM_OPENMP=1 binary-level reverse-compat NOT preserved due to Kahan injection
- D11 tag-immutability preserved (P1-update-omp-tag SHA unchanged)
EOF
)" \
...
**Trade-off accepted** (per master plan §3 fallback option 2 + spec L100-L103 carve-out Scenario): 为换 heihe Δ_nst 225 → 84 (~63% 改善), 牺牲 N=1 binary-level reverse-compat。Kahan patch held-in-reserve (`docs/p1c_kahan_patch.diff`), 可 P9 stage NUMA 治理后回退 + N=1 reverse-compat 恢复。

## §9 限制与已知问题

1. **N≥4 bit-level A3a 仍 FAIL** — P1c 阶段未关闭, 推 P9 (per §1 carve-out scope + §5.2 P9 hand-off)。
...
3. **heihe_x4 Δ_wall 异常改善** — Kahan-injected wall 比 pre-Kahan 减少 15-23%, 与 R2 +1-3% perf 降估算相反。需 PR-K capstone 后 P2a/P9 阶段 cross-check P1 era PR-K1 wall baseline 是否在同 hardware platform 上同样幅度。
...
| 文档 | 内容 |
|---|---|
| [`docs/p1c_summary.md`](p1c_summary.md) | 本文件, P1c capstone source of truth |
| [`docs/p1c_a3a_root_cause.md`](p1c_a3a_root_cause.md) | A3a 根因分析 + decision branch + Kahan 候选路径 + 终判 |
| [`docs/p1c_perf_baseline.md`](p1c_perf_baseline.md) | Mac 16-cell + Server PR-H/PR-I 8-cell wall + 数据 |
| [`docs/p1c_reduction_sites.md`](p1c_reduction_sites.md) | 8-site reduction overview |
| [`docs/p1c_b1b_serial_order_dump.txt`](p1c_b1b_serial_order_dump.txt) | B1b 基线 serial order dump |
| [`docs/p1c_pr_h_server_first_run.md`](p1c_pr_h_server_first_run.md) | PR-H server first run 8-cell raw data |
| [`docs/p1c_pr_i_kahan_injection.md`](p1c_pr_i_kahan_injection.md) | PR-I Kahan 二跑 8-cell raw data + carve-out 决策 |
| [`docs/p1c_pr_j_reverse_compat.md`](p1c_pr_j_reverse_compat.md) | PR-J reverse-compat 双视角 |
| [`docs/p1c_kahan_patch.diff`](p1c_kahan_patch.diff) | Kahan held-in-reserve patch (PR-G 输出 + PR-I 应用) |
