...
_Avoid_: 把 Kahan held-in-reserve 与 classic Kahan summation 混淆（Neumaier 显式处理 sign-mixing 序列，对站点 5/6/7 sigma 含负数 robust）；把 patch apply 后 SHUD pin 3a0004c 当作 baseline/P1c 的永久 pin（P1d NUMA 治理后可 revert 回 de9545d 等价 pre-Kahan helper-wrap，per `docs/p1c/p1c_pr_j_reverse_compat.md` §4 rollback option）
...
_Avoid_: 把 P1d carve-out 与 P7 final-fusion deterministic-reduction 混淆（两 forward debt 独立：P7 = fork-join + chunk-fixed schedule；P1d = NUMA + first-touch governance）；把 P1d carve-out 与 §6 P9 production deterministic reduction 混淆（P1d 是 strict 模式 + bitwise B1b，§6 P9 是 production 模式 + 允许偏离 B1b，且必须晚于 P7；命名空间不重叠）；把 P1d work scope 误以为可 P2a 阶段解决（master plan §6 P1d.1-6 显式范围）
