...
**nfeLS/nfe ratio**:
SUNDIALS-CVODE Krylov amplification 比率 = `nfeLS` (Krylov 线性求解器 ƒ-evaluation 次数, via matrix-free DQ Jacobian-vector product) ÷ `nfe` (Newton 非线性 ƒ-evaluation 次数, via CVODE integrator 主动调用 RHS)。p8pre-spike Step 1 PREC_NONE canonical 实测：heihe N=8 `r = 12632/6943 = 1.819` (=`r_min`), heihe_x4 N=8 `r = 30518/6741 = 4.526` (=`r_max`), 均超 ADR-0002 Path 3 trigger threshold `r ≥ 1.5` → branch a (PROCEED Step 2)。**ADR-0002 Path 3 trigger criterion**：`r ≥ 1.5` 是 SPGMR Krylov 工作量足以摊销 preconditioner setup overhead 的 ROI 阈值, 低于此值 preconditioner 投入 negative ROI。详 `docs/p8pre/n8_profile_verdict.md` §5。
