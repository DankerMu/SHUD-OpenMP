...
**p8pre-tag**:
Annotated git tag locking `baseline/p8pre` HEAD at commit `e442ce8` (打于 2026-06-28，message 含 "epic p8pre (PR-A→PR-G) capstone + SHUD pointer bump 5276167→37be0fe (design D8 cleanup); ADR-0003 verdict accepted; D11 chain successor to P1e-tag")。Capstone commit 是 SHUD pointer bump 把 SHUD submodule 从 identity-precond-stub `5276167` 还原到 cleaned-PREC_NONE `37be0fe` (per ADR-0003 NO-GO option (b) design D8 fall-back)。**D11 chain position**: 9th tag (B0 → B1a → B1b → B1 → P1-update-omp → P1c → P1d → P1e → **p8pre**)，post-2026-06-28 update。**Baseline 分支**: `baseline/p8pre` (lock_branch=true via D11 protection 同 P1e 模式)。详 epic #338 archive `openspec/changes/archive/p8pre-spike/`、`docs/p8pre_summary.md`、`docs/adr/0003-precond-spike-decision.md`、master plan §P8-precond.0 D11 chain stacking row + §P1e.5 forward-compat note。
_Avoid_: 把 `p8pre-tag` 当作 P8-precond production epic 启动 anchor（实际是 spike NO-GO capstone, future preconditioner candidate 启动需新 epic-tag）；移动 tag 到新 commit（annotated tag 一经 push 即 immutable，新 capstone 用新 tag 如 `p8precond-vN-tag` stacking）
...
**p8tune-tag**:
Annotated git tag locking `baseline/p8tune` HEAD at commit `c8b2ef7` (打于 2026-06-28，message 含 "epic #362 p8tune-spgmr-maxl capstone (PR-0→PR-E aggregator+ADR-0004); merge_sha 56dd70f4; review-loop-log finalized; D11 chain successor to p8pre-tag")。Capstone commit 是 review-loop-log finalize entry, 标志 6-PR epic (PR-0 doc-correction → PR-A clean-PREC_NONE-baseline → PR-B verdict-gate → PR-C spgmr-maxl-env-hook → PR-D 60-cell-sweep → PR-E aggregator+ADR-0004) 全部 merged。ADR-0004 verdict = Optional-knob branch (`SHUD_SPGMR_MAXL=30` for heihe N=1 = +11.99% wall; **Performance opt-in tier NOT A5-certified** per PR #378 NumY 口径 doc-correction amendment)。**D11 chain position**: 10th tag (... → p8pre → **p8tune**)，post-2026-06-28 update。**Baseline 分支**: `baseline/p8tune` (lock_branch=true via D11 protection)。**Forward action**: ADR-0004 §Forward action 指 P9-A5 hydrology equivalence epic (promote maxl=30 from opt-in → A5-certified) + P8-tune.D KLU pattern-only spike epic (epic #379 staged 2026-06-28)。详 epic #362、ADR-0004、`docs/p8tune/maxl_sweep_verdict.md`、master plan §P8-tune.C [CLOSE 2026-06-28]。
_Avoid_: 把 `p8tune-tag` 当作 large-case acceleration 完成 anchor（实际 heihe_x4 全 maxl≥10 wall REGRESS, SPGMR-tune saturated for production scale, 需 P8-tune.D KLU spike epic 答 GO/NO-GO）；把 maxl=30 当 A5-certified default（仍是 Performance opt-in tier, A5 promote 待 P9-A5 epic 完成）；移动 tag 到新 commit（同 p8pre-tag 不变性约束）

### P11-osc (minute-scale stepping diagnosis) 集合

P11-osc epic (openspec change `p11-osc`, epic #433, PR-D1 #437 / PR-D2 #438 / PR-D3 #439, 2026-07-01→02) 诊断 qinyijiang/keliya 分钟级 CVODE 步进成因（数值振荡 vs 真实动力学）。裁决 = **REAL_DYNAMICS**（`docs/p11-osc/diagnosis_verdict.md`，control sanity PASS），optimization/limiter 线关闭；分支 `research/p11-osc`（outer）+ `p11-osc`（SHUD，post-verdict merge-back `openmp-baseline`）。

**P11-osc verdict enum**:
`{OSC_CONFIRMED, REAL_DYNAMICS, INCONCLUSIVE}` — spike kill-gate 的 TOTAL 决策函数输出（每个输入组合恰映射一个 verdict；阈值 pinned 在 osc-diag-analyzer spec + spike_brief §Kill-gate，nothing decided in-tool）。OSC_CONFIRMED = burst_share_60s ≥ 0.50 ∧ top-1% concentration ≥ 0.50 ∧ per-day Spearman ρ ≥ 0.5（仅此分支允许提案 ONE limiter candidate，design-only + A5 tightened）；REAL_DYNAMICS = burst < 0.50 ∨ concentration < 0.50（空间弥散 → 关闭 optimization 线）；INCONCLUSIVE = burst ≥ 0.50 ∧ conc ≥ 0.50 ∧ ρ < 0.5（→ 至多一次 refinement 后强制 re-verdict）。Epic 裁决聚合规则：= qinyijiang MARKER verdict，仅在 control（xinanjiang_upstream）burst_share_60s < 0.05 sanity PASS 时有效；keliya 仅 corroborating。2026-07-02 实测：qinyijiang burst=0.9942 / conc=0.0366 / ρ=−0.3753 → REAL_DYNAMICS（concentration disjunct 驱动）。
_Avoid_: 把 REAL_DYNAMICS 误读为 "burst 不高"（qinyijiang burst=0.9942 极高——分钟级步进真实存在，判 REAL_DYNAMICS 是因翻转空间弥散非局部化）；在非 OSC_CONFIRMED 下提案 limiter（kill-gate 硬约束，falsify physics）。

**SHUD_DIAG_DT / SHUD_DIAG_OSC**:
SHUD 侧 read-only 诊断 env gates（PR-D1 #437，`SHUD/src/Model/MD_osc_diag.hpp` + `shud.cpp` 3 call sites，SHUD commit `75afb2b`）。STRICT `=1` 谓词（getenv + strcmp 等于 "1"；presence-only 或 `=0` 不启用——比 `SHUD_DUMP_CV_Y` presence-only 前例更严，placement 复用、predicate 不复用）。`SHUD_DIAG_DT=1` → per-SolverStep-interval CVODE counter deltas（`diag_dt_trace.csv`：t_next,delta_nst,delta_nfe,delta_ncfn,delta_netf,h_last,h_cur + header project_name/solverstep_min）；`SHUD_DIAG_OSC=1` → accepted-boundary state-delta sign-flip counters（`diag_osc_flips.csv` typed ele|riv rows + `diag_osc_flips_daily.csv` day_index=floor(t_next/1440)；epsilon_m=1e-6 m dead-floor sign-holding）。状态读 `N_VGetArrayPointer(udata)` accepted 状态（layout [sf|us|gw|riv] per Macros.hpp iSF/iUS/iGW/iRIV），绝不碰 `uYsf`/`uYus`/`uYgw` RHS scratch globals（P1e PR-B0 hazard class）。默认路径 bitwise-neutral（keliya B0 四腿 SHA 证据，leg-c 为 strict-=1 的 load-bearing 证据）。
_Avoid_: presence-check 谓词（leg-c 证据专门反证）；从 uY* scratch globals 读状态（持有 t_internal ≠ tout 的 trial-state）；把 daily CSV 的 N+1 行栅栏（trailing partial-day bucket，t_next=END·1440 恰落 day END）当 bug（spec-faithful；analyzer 对 Spearman join 双侧对称丢弃处理）。

**MARKER:OSC_DIAG_VERDICT**:
`tools/osc_diag/` analyzer（PR-D2 #438，uv 项目，51 pytest）stdout 的机器可读裁决块（`MARKER:OSC_DIAG_VERDICT_BEGIN..END`，含 case/verdict/burst_share_60s/burst_share_10s/top1pct_concentration/spearman_rho + thresholds）。FAIL-CLOSED：malformed/missing/truncated 输入 → exit 2 + 零 MARKER 输出，绝不从部分数据产 verdict。伴生 artifacts：`dt_histogram.csv`（半开区间 bins [0,10),[10,60),[60,300),[300,600),[600,1200),[1200,∞)）+ `osc_diag_summary.json`。`--case` 显式映射 project_name→benchmark case（nanlin→qinyijiang，basin folder ≠ project name）。
_Avoid_: 只解析 stdout 不检查 exit code（fail-closed 语义靠双重信号）；hardcode solverstep_min（必须读 trace header，R5 风险）。

**burst share / flip counters / D1-D2-D3**:
P11-osc 三诊断标签。**D1** = per-CVode-call dt trace via counter deltas（NOT CV_ONE_STEP、NOT SUNDIALS monitor——两者被 design 显式拒绝）；**D2** = accepted-boundary state-delta sign-flip counters（SolverStep 分辨率 proxy，under-count 保守方向——bias against OSC_CONFIRMED，never toward）；**D3** = analyzer + verdict gate。**burst_share_60s** = nst 加权的 sub-60s 份额（Σ delta_nst over mean_dt<60s intervals ÷ Σ delta_nst；delta_nst=0 行从分子分母双侧跳过）；**mean_dt_seconds** = 60·solverstep_min/delta_nst（canonical 公式，minutes→seconds 换算归 analyzer）。**flip** = 相邻 interval state delta 符号交替（rise→fall→rise）；per-element flips = sf+us+gw 求和（elements-only 入 top-1% concentration 分母），riv rows 独立报告不入分母。
_Avoid_: 把 burst share 当 interval-count 加权（是 nst 加权）；把 D2 under-count 当利于 OSC_CONFIRMED 的偏差（保守方向恰相反）；用 flip 计数跨 ele/riv 混合排名（separate index spaces）。

### P12-nvec (deterministic hybrid NVector) 集合

P12-nvec epic (openspec change `p12-nvec`, epic #441, PR-N0 #447 / PR-N1 #448 / PR-N2 #TBD, 2026-07-02) 抬升 Config C 的 Amdahl 天花板（heihe_x4 sp@16=1.946×, serial fraction≈48%）——用 OpenMP 并行 CVODE 内部 NVector **element-wise** ops、reductions 全部保持 serial，保 release 保证（跨线程数 + vs serial baseline **bitwise identical**）；Tier 2（gated）才用 fixed-tree deterministic reductions 换 reductions 并行（一次性 controlled re-baseline）。分支 `research/p12-nvec`（outer）+ `p12-nvec`（SHUD, post-verdict merge-back `openmp-baseline`）。**PR-N2 裁决（2026-07-02，heihe_x4 90-day matrix）= TIER1_ADOPT（speedup(E/C)=1.4124×@N16 / 1.3094×@N8 ≥ 1.10× bar, bitwise PASS at all N）+ TIER2_GO（三输入全 PASS: ADOPT ∧ reduction share 35.68%/30.46%≥10% ∧ Amdahl proj 1.3744×≥1.15×）→ Config E opt-in build leg, PR-N3 fixed-tree unblocked**。裁决入 ADR-0011 + `docs/p12-nvec/tier1_verdict.md`。

**Config E**:
本 epic 的 Tier-1 hybrid NVector build leg：`make shud_omp SHUD_USE_OPENMP_NVECTOR=1 SHUD_NVEC_HYBRID=1`。在 `N_VNew_OpenMP(NY, cfg_NUM_OPENMP, sunctx)` 创建后、`CVodeInit` 前，用 SHUD-owned serial loop（generic API：`N_VGetArrayPointer` + `N_VGetLength`，绝不调 `*_Serial` on OpenMP-content vector）overwrite reduction 类 ops（`src/Model/MD_nvec_hybrid.{hpp,cpp}`）。element-wise ops 保 stock OpenMP 并行。determinism = 期望跨线程数 + vs Config C **bitwise identical**（A3a-strength），因 element-wise 每元素 FP order 与 Serial 一致、reductions 走同一 serial 左→右序。区别于 **Config C**（release：StrictOMP RHS + Serial NVector）与 **Config D**（refuted：StrictOMP RHS + stock OpenMP NVector，`reduction(+:sum)` 序随线程数漂移 10-25%）。
_Avoid_: 把 Config E 当 "新 NVector backend"（是 ops-table override，~150 LOC，不 fork nvector_openmp.c）；把 Config E 与 Config D 混淆（E 的 reductions serial，D 的 reductions stock-OpenMP → E bitwise / D drift）；在 OpenMP-content vector 上调 `NV_DATA_S`/`*_Serial`（content-struct layout aliasing = UB）。

**Config E2 / SHUD_NVEC_DETRED**:
Tier-2 conditional build leg（仅 TIER2_GO 后）：`make shud_omp SHUD_USE_OPENMP_NVECTOR=1 SHUD_NVEC_HYBRID=1 SHUD_NVEC_DETRED=1`。`SHUD_NVEC_DETRED`（compile-time flag，default 0；requires `SHUD_NVEC_HYBRID=1` else loud build error）把 Tier-1 serial reduction overrides 换成 **fixed-tree deterministic reductions**：block size B 编译期常量（e.g. 4096，与线程数无关），block 内 serial index-order 累加，block partials 按 FIXED binary-tree 序 combine，thread→block 映射 dynamic 不影响 combine 序。flag=0 → Config E 行为 byte-identical to pre-PR-N3（revert = 不带 flag 重编，非 git surgery）。cross-thread bitwise by construction；但 summation order 相对 Config C serial 变一次 → 一次性 re-baseline + A5 certification of the new reference。
_Avoid_: 把 SHUD_NVEC_DETRED 当 runtime env var（是 compile-time flag）；把 Config E2 与 Config E 当同一 reference（E2 一次性 summation-order shift, A3a 不跨 shift 适用、只在 new reference 内适用, mode-C-tune 先例）；`SHUD_NVEC_DETRED=1` 不带 `SHUD_NVEC_HYBRID=1`（loud build error，不 silent no-op）。

**SHUD_NVEC_HYBRID**:
Config E 的 compile-time flag（default 0；meaningful 仅当 `SHUD_USE_OPENMP_NVECTOR=1`；Makefile 在 HYBRID=1 无 NVector flag 时 `$(error ...)` fail loud）。=1 → `-DSHUD_NVEC_HYBRID=1` 触发 `shud.cpp` 里 `nvec_hybrid_install(udata)`/`nvec_hybrid_install(du)` 在 `N_VNew_OpenMP` 后 install serial reduction overrides。default builds（`make shud` / `make shud_omp`）不带 → CI build-and-compare green；f.cpp / MD_rhs_core.cpp zero diff。
_Avoid_: 把 SHUD_NVEC_HYBRID 当运行期开关（compile-time only）；以为它单独就能启用（须与 SHUD_USE_OPENMP_NVECTOR=1 同设）；以为它改 RHS（RHS 路径零改动，只碰 NVector ops table）。

**SHUD_NVEC_PROF**:
PR-N0 引入的 NVector op-share profiler 的 runtime env gate（STRICT `=1` 谓词：`getenv && strcmp(v,"1")==0`；presence-only 或 `=0` 不启用——P11-osc predicate discipline）。=1 → 在 vector 创建时（overrides 之后、profiler wraps last/outermost）用 counting + monotonic-ns shim 包每个 populated op pointer，纯 delegation，clone 传播携带 shim 到所有 CVODE-internal temporaries，run 结束 dump `nvec_prof.csv`（rows `op_name,op_class{elementwise|reduction|other},calls,total_ns`；header `project_name/NY/nthreads/backend{serial|openmp|hybrid}`）。默认路径 bitwise-neutral（gate off = stock behavior）。normative overhead gate ≤ 2%（heihe_x4 Config C N=16 3-run median，实测 −0.117% PASS）。**用途**：算 t_CVODE_raw 的 elementwise / reduction / non-NVector 份额，供 G-E3(ii)/(iii)。
_Avoid_: presence-check 谓词（strict `=1`，PR-N0 leg-c load-bearing）；把 profiler 当 sampling profiler（是 ops-table wrapper，measure exactly 要并行的边界）；把 shim distortion（~1% a-priori）与 G-E3 10% 门限混为一谈（差一个量级，且 gate 吃 shares 不吃 absolutes）。

**G-E1 / G-E2 / G-E3 / G-E4**:
P12-nvec 四门控。**G-E1**（Tier-1 bitwise, HARD, PR-N1）= keliya Config E N∈{1,2,4,8} 全部 SHA-identical to each other AND to Config C baseline；mismatch → element-wise bug（fix）或 unlisted reduction（extend override list），Tier-1 内无 tolerance fallback。**G-E2**（Tier-1 ROI, PR-N2）= heihe_x4 server matrix N∈{1,8,16}，`TIER1_ADOPT iff speedup(E/C) ≥ 1.10 at N=8 or N=16 AND bitwise PASS, else TIER1_CLOSE`（bar 1.10× pinned；vs P9 1.5× 因 Tier-1 trajectory-identical、零 re-qualification）。**G-E3**（Tier-2 GO, PR-N2）= ALL of (i) Tier-1=TIER1_ADOPT (ii) reduction share ≥ 10% on heihe_x4 OR heihe_x16（each pinned Config C N=16 profile leg）(iii) Amdahl projection ≥ 1.15× additional wall over Config E at N=16，公式 `wall_E16 / (wall_E16 − t_red·(1 − 1/16))`；任一 fail → TIER2_NO-GO, ZERO Tier-2 code。**G-E4**（Tier-2 acceptance, 仅 GO, PR-N3）= cross-thread bitwise（keliya N∈{1,2,4,8} + forced-small-B leg + heihe_x4 cross-N N∈{8,16}）→ A4 max_ulp report → full A5 with tightened `a5_thresholds.p12_tier2.yaml`；determinism first, accuracy second。
_Avoid_: 把 G-E1 当 ROI gate（是 bitwise HARD gate）；移动 G-E2 1.10× 或 G-E3 1.15× bar（pinned in spec + design, verdict doc 不得动）；把 G-E3(ii) 当 "两 case 都要 ≥10%"（either/OR：任一 ≥ 10% 即 (ii) PASS）；跳过 G-E4 的 determinism-first 序（cross-thread bitwise 必须先于 A4/A5）。

**fixed-tree reduction**:
Tier-2（Config E2）的 reduction 机制：把 `[0, NY)` 分成 FIXED size B（编译期常量，与线程数无关）的 blocks，每 block serial index-order 累加，block partials 按 FIXED binary-tree 序 combine，thread→block 映射 dynamic。cross-thread bitwise **by construction**（combine 序是 NY 与 B 的纯函数，与线程数无关）。相对 Config C 的 serial 左→右单次求和序变一次 → 一次性 re-baseline。Optional Neumaier compensation（legacy §P9 menu）按 PR-N3 实测精度需求决定。
_Avoid_: 把 fixed-tree 当 "OpenMP reduction 的 deterministic 版"（OpenMP `reduction(...)` 序随线程数变——正是 Config D 失败源；fixed-tree 序恒定）；以为 B 依赖线程数（B 编译期常量，thread→block 映射才 dynamic）；把 summation-order shift 当并行 bug（legacy §P9："偏离 baseline 不是错误"——但须与并行 bug 分离，G-E4 determinism-first）。

**TIER1_ADOPT / TIER1_CLOSE / TIER2_GO / TIER2_NO-GO**:
P12-nvec 两级 TOTAL 决策函数输出（每输入组合恰映射一 verdict；阈值 pinned in spec/design/spike_brief，nothing decided in-tool）。**TIER1_ADOPT** = G-E2 PASS（speedup ≥ 1.10 at N=8|N=16 ∧ bitwise PASS）→ Config E 作 opt-in build leg，release-tag promotion 推 follow-up。**TIER1_CLOSE** = else → code default-off，revert/keep 决定入 ADR-0011。**TIER2_GO** = G-E3 三输入全 PASS → lift PR-N3 blocked-by。**TIER2_NO-GO**（first-class outcome）= 任一 G-E3 输入 fail → close PR-N3 issue as not-planned（gate-values 评论），ZERO Tier-2 code。本 epic 实测 verdict 见 ADR-0011 + `docs/p12-nvec/tier1_verdict.md`。
_Avoid_: 把 TIER2_NO-GO 当 "失败"（是 first-class 结果，mirrors P11-osc "confirmed path creates no implementation"）；在 TIER1_CLOSE 或非-GO 下写 Tier-2 code（gate 硬约束）；把 Tier-1 ADOPT 当 release default（Config E 是 opt-in，release-tag promotion 是 post-epic follow-up）。
